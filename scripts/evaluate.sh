...
for LG in archimob1 gos1 skn1 ndc1 skn2 ndc2 skn3 ndc3; do
    for SPLIT in a b c ; do
        ...
        python3 align.py ../processed_data/"$LG$SPLIT"/dev.src ../models/$MODEL/test_predictions/"$LG$SPLIT"/test_pred.tgt ../data/"$LG$SPLIT"/dev.norm ../models/$MODEL/test_predictions/"$LG$SPLIT"/test_aligned.txt

        # word-level evaluation with normEval.py
        python3 normEval.py --gold ../data/"$LG$SPLIT"/dev.norm --pred ../models/$MODEL/test_predictions/"$LG$SPLIT"/test_aligned.txt >> eval_results.txt
        echo "" >> eval_results.txt
    done
done
