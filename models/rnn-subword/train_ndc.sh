...
#SBATCH -J rnn-ndc
#SBATCH -o rnn-ndc.%j.out
#SBATCH -e rnn-ndc.%j.err
...
for LG in ndc3b; do
	echo $LG
	sed 's/LG/'"$LG"'/g' < config_ndc.yaml > config_"$LG".yaml
	...
	onmt_translate -model $LG/model_best.pt -src ../data/$LG/dev_unigram2000.src -output $LG/dev_pred.tgt -replace_unk -max_length 1000 -min_length 1 -length_penalty wu -alpha 0.6
done
